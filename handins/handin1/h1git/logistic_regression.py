...
class LogisticRegressionClassifier():
    ...
    def cost_grad(self, X, y, w):
        ...
        d = np.shape(X)[1]
        n = np.shape(X)[0]
        cost = np.mean(np.log(1+np.exp((-y)*np.dot(X,w))))
        grad = np.reshape(np.mean(X*((-y)*logistic((-y)*np.dot(X,w))).reshape(n,1),axis=0),(d,))
        #grad = np.reshape(np.mean(X*(logistic(X@w*(-y))*(-y)).reshape(n,1),axis = 0),(d,))
        ### END CODE
